...
class Element:
    ...
    def __init__(self,problem_parameters):
        ...
        self.simd="-Ofast",
        self.Olevel="avx512",
        self.problem_size1=problem_parameters[0]
        self.problem_size2=problem_parameters[1]
        self.problem_size3=problem_parameters[2]
        self.threads = problem_parameters[3]
        self.cache1 = problem_parameters[5]
        self.cache2 = problem_parameters[6]
        self.cache3 = problem_parameters[7]
        self.iter = problem_parameters[4]
        self.code = ()
...
def cmdMake( element : Element = None, process = 0, is_display : bool = False):
    try:
        ...
        checking_path = f"~iso3dfd-st7/iso3dfd_dev13_cpu{element.Olevel}_{element.simd}.exe"
        ...
        if not is_file:
            ...
            #Renaming the file
            current_path = "~bin/iso3dfd-st7/iso3dfd_dev13_cpu_"+element.simd+".exe"
...
def cmdRunIso (element , process = 0, is_display : bool = False):
    ...
    cmd = path + " " \
            ...
            + str(element[3]) + " " \
            + str(element[4]) + " " \
            + str((element[5]//16)*16) + " " + str(element[6]) + " " + str(element[7])
    flops = -1
    iterations = 0
    time = -1
    while flops == -1 and iterations < 5:
        if is_display:
            display_element_process(Element(element), process, "Running program")
        result = subprocess.run(cmd, shell="True", stdout=subprocess.PIPE, text=True)
        flops, mpoints, time = get_output_info(result)
        iterations += 1
       
    mark = BenchMarkValue(time, mpoints, flops, Element(element))
    if is_display:
        display_element_process(element, process, mark)
    return mark   
